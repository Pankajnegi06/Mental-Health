...
export const LeftSide = ({ room, email }) => {
  ...
  const handleCallUser = useCallback(async () => {
    console.log("📞 Calling user with socket ID:", remoteSocketId);
    toast.success("Call sent. Waiting for response...");
    ...
    try {
      ...
      // Request media after sending the call
      const stream = await navigator.mediaDevices.getUserMedia({
        audio: true,
        video: true,
      });
  ...
  const getAllUserList = useCallback((allUsers) => {
    ...
    for(let ele of allUsers){
      if(ele.socketid !== socket?.id){
        console.log("Found remote user:", ele.username, ele.socketid);
        setEnemy(ele.username);
        setRemoteSocketId(ele.socketid);
      }
    }
  }, [socket]);
  ...
  // Request user list when component mounts
  useEffect(() => {
