...
const register = async (req, res) => {
  ...
  try {
    ...
    //sending welcome MAIL message - non-blocking
  ...
  `,
    ...
    transporter.sendMail(mailDetails).catch((error) => {
...
const isAuthenticated = async (req, res) => {
  try {
    ...
    // User and token should be attached by middleware
    ...
    const user = await myModel.findById(userId).select('-password');
    if (!user) {
    ...
    return res.json({ 
      ...
      token: token, // Return token so frontend can save it
      user: {
        _id: user._id,
        name: user.name,
        email: user.email
...
const verifyOTPforPasswordReset = async (req, res) => {
  ...
  `,
    ...
    // Send password reset OTP email asynchronously without blocking
    transporter.sendMail(mailOptions).catch((error) => {
